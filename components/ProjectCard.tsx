...
const ProjectCard: React.FC<ProjectCardProps> = ({ project }) => {
  const { langKey, t } = useTranslation();
  ...
  return (
    <div className="bg-slate-800/50 rounded-lg p-6 flex flex-col ring-1 ring-slate-700/50 hover:ring-sky-400/50 transition-all duration-300 ease-in-out transform hover:-translate-y-1 shadow-lg hover:shadow-sky-500/10">
      <div className="flex-grow">
        <h3 className="text-xl font-bold text-slate-100 mb-2">{project.title[langKey]}</h3>
        <p className="text-slate-400 mb-4 leading-relaxed">{project.description[langKey]}</p>
        <div className="mb-4">
      ...
      <div className="mt-auto flex flex-col sm:flex-row gap-4">
        ...
        >
          <ExternalLinkIcon />
          {project.liveButtonText[langKey]}
        </a>
        <a
